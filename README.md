...
## pos and vel interpol

    void moveAllPosition(const std::vector<std::vector<double>>& trajectories,
        ...
        for (size_t step = 0; step < trajectories.size(); step++) {
            ...
            while (ros::Time::now().toSec() < end_time) {
                ...
                for (int j = 0; j < 12; j++) {
                    ...
                    lowCmd.motorCmd[j].Kp = 5;  // Adjust as needed
        ...
        for (size_t step = 0; step < trajectories.size(); step++) {
            ...
            while (ros::Time::now().toSec() < end_time) {
                ...
                for (int j = 0; j < 12; j++) {
                    ...
                    lowCmd.motorCmd[j].Kp = 0;  // Set to 0 for velocity control
                    lowCmd.motorCmd[j].Kd = 10;  // Adjust as needed for your robot
...
## torque control
    ...
    void moveAllPositionsTorqueControl(const std::vector<std::vector<double>>& torques,
                                       const std::vector<double>& durations)
    {
